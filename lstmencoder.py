...
def LSTMDataGenerator(data, lead_time,seq_length,max_lead_time):
...
def train_lstm(encoder,decoder,lr,total_epoch,train_loader,data_valid_x,data_valid_y,model_save_fn,max_lead_time,lead_time, batch_size, hidden_size,num_layers):
    if (os.path.exists(model_save_fn)):
        print('model has existed')
    else:
        sumWriter = SummaryWriter('lstmencoder_log')
        ...
        for epoch in range(total_epoch):
            for step, (x, y) in enumerate(train_loader):
                ...
                train_loss=loss_func(decoderout, y)
                optimizer.zero_grad()
                train_loss.backward()
                optimizer.step()
            ...
            decoderout, decoderout_state = decoder(decoder_input)
            decoderout = decoderout.squeeze(-1)
            valid_loss = loss_func(decoderout, data_valid_y)
            # ########################################################
...
def main(datadir,dataname,hidden_size,num_layers,lr,total_epoch,batch_size,lead_time,
    ...
    [data_x, data_y,data_z] = LSTMDataGenerator(data, lead_time, seq_length,max_lead_time)
    ...
    encoder, decoder=train_lstm(encoder,decoder,lr,total_epoch,train_loader,data_valid_x,data_valid_y,model_save_fn,max_lead_time,lead_time, batch_size, hidden_size,num_layers)
